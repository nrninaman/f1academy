        ...
        <?php if (!empty($races)): ?>
            <?php foreach ($races as $race): ?>
                <div class="bg-gray-800 p-6 rounded-xl shadow-lg border-l-4 <?php echo $race['is_completed'] ? 'border-green-500' : 'border-blue-500'; ?>">
                    <h2 class="text-3xl font-bold mb-2"><?php echo htmlspecialchars($race['name']); ?> (Round <?php echo htmlspecialchars($race['round_number']); ?>)</h2>
                    <p class="text-gray-400 mb-4">Date: <?php echo date('F d, Y', strtotime($race['date'])); ?></p>
                    <p class="text-lg">**Details:** <?php echo htmlspecialchars($race['details'] ?? 'No specific details provided for the weekend format.'); ?></p>
